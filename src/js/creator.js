...
class DiscordEmbedCreator {
    ...
    bindEvents() {
        ...
        document.getElementById('mobile-save-template').addEventListener('click', () => {
            this.showSaveModal();
        });

        document.getElementById('mobile-load-template').addEventListener('click', () => {
            this.showLoadModal();
        });
        ...
        document.getElementById('mobile-import-json').addEventListener('click', () => {
            this.importJSON();
        });
    ...
    handleJSONFileSelect(event) {
        ...
        reader.onerror = () => {
            this.showNotification('Failed to read the file. Please try again.', 'error');
        };
    ...
    loadJSONData(jsonData) {
        try {
            ...
            // Load webhook URL and content if present
            if (jsonData.webhook) {
                document.getElementById('webhook-url').value = jsonData.webhook;
                this.embedData.webhook = jsonData.webhook;
            }

            if (jsonData.content) {
                document.getElementById('message-content').value = jsonData.content;
                this.embedData.content = jsonData.content;
            }

            // Load embed data if present
            if (jsonData.embeds && jsonData.embeds.length > 0) {
                const embed = jsonData.embeds[0]; // Load first embed
                this.loadEmbedFromData(embed);
            }
            ...
            this.showNotification('Error loading embed data. Please check the JSON structure.', 'error');
        }
    }

    loadEmbedFromData(embed) {
        // Basic embed properties
        if (embed.title !== undefined) {
            document.getElementById('embed-title').value = embed.title;
            this.embedData.title = embed.title;
        }

        if (embed.description !== undefined) {
            document.getElementById('embed-description').value = embed.description;
            this.embedData.description = embed.description;
        }

        if (embed.url !== undefined) {
            document.getElementById('embed-url').value = embed.url;
            this.embedData.url = embed.url;
        }

        if (embed.color !== undefined) {
            const colorHex = '#' + embed.color.toString(16).padStart(6, '0');
            document.getElementById('embed-color').value = colorHex;
            this.embedData.color = colorHex;
        }

        // Author
        if (embed.author) {
            document.getElementById('author-enabled').checked = true;
            this.toggleSection('author-enabled');
            
            if (embed.author.name) {
                document.getElementById('author-name').value = embed.author.name;
                this.embedData.author.name = embed.author.name;
            }
            if (embed.author.url) {
                document.getElementById('author-url').value = embed.author.url;
                this.embedData.author.url = embed.author.url;
            }
            if (embed.author.icon_url) {
                document.getElementById('author-icon').value = embed.author.icon_url;
                this.embedData.author.iconUrl = embed.author.icon_url;
            }
        }

        // Thumbnail
        if (embed.thumbnail && embed.thumbnail.url) {
            document.getElementById('thumbnail-enabled').checked = true;
            this.toggleSection('thumbnail-enabled');
            document.getElementById('thumbnail-url').value = embed.thumbnail.url;
            this.embedData.thumbnail.url = embed.thumbnail.url;
        }

        // Image
        if (embed.image && embed.image.url) {
            document.getElementById('image-enabled').checked = true;
            this.toggleSection('image-enabled');
            document.getElementById('image-url').value = embed.image.url;
            this.embedData.image.url = embed.image.url;
        }

        // Footer
        if (embed.footer) {
            document.getElementById('footer-enabled').checked = true;
            this.toggleSection('footer-enabled');
            
            if (embed.footer.text) {
                document.getElementById('footer-text').value = embed.footer.text;
                this.embedData.footer.text = embed.footer.text;
            }
            if (embed.footer.icon_url) {
                document.getElementById('footer-icon').value = embed.footer.icon_url;
                this.embedData.footer.iconUrl = embed.footer.icon_url;
            }
        }

        // Timestamp
        if (embed.timestamp) {
            document.getElementById('footer-timestamp').checked = true;
            this.embedData.footer.timestamp = true;
        }
        ...
        if (embed.fields && embed.fields.length > 0) {
            ...
            embed.fields.forEach(field => {
                this.addField();
                const fieldIndex = this.embedData.fields.length - 1;
                const fieldElement = fieldsContainer.children[fieldIndex];
                
                fieldElement.querySelector('.field-name').value = field.name || '';
                fieldElement.querySelector('.field-value').value = field.value || '';
                fieldElement.querySelector('.field-inline').checked = field.inline || false;
                
                this.embedData.fields[fieldIndex] = {
                    name: field.name || '',
                    value: field.value || '',
    ...
    resetForm() {
        ...
        // Reset toggles
        const toggles = ['author-enabled', 'thumbnail-enabled', 'image-enabled', 'footer-enabled', 'footer-timestamp'];
        ...
        document.getElementById('author-url').value = '';
        document.getElementById('author-icon').value = '';
        ...
        document.getElementById('footer-icon').value = '';
